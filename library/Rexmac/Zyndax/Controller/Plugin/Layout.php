...
class Layout extends \Zend_Controller_Plugin_Abstract {
  ...
  public function dispatchLoopStartup(AbstractRequest $request) {
    ...
    #if($moduleName === 'default' && $userCanViewAdmin) $moduleName = 'admin';
    if('default' !== $moduleName) {
      $fc = FrontController::getInstance();
      $moduleConfig = $fc->getParam('bootstrap')->getResource('modules')->offsetGet($moduleName)->getOptions();
      if(isset($moduleConfig['resources']['layout']['layoutPath'])) {
        Zend_Layout::getMvcInstance()->setLayoutPath($moduleConfig['resources']['layout']['layoutPath']);
      }
    }
  }
}
