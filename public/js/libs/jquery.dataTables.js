...
(/** @lends <global> */function($, window, document, undefined) {
	...
	var DataTable = function( oInit )
	{
		...
		function _fnBuildHead( oSettings )
		{
			...
			if ( iThs !== 0 )
			{
				...
				for ( i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
				{
					...
					nTh.innerHTML = '<span>' + nTh.innerHTML + '</span><span class="sort-icon"></span>';
				}
			}
			else
			{
		...
		function _fnDraw( oSettings )
		{
			...
			if ( oSettings.nTBody )
			{
				...
				for ( i=0, iLen=anRows.length ; i<iLen ; i++ )
				{
					...
					if(anRows[i].length) {
						nAddFrag.appendChild(anRows[i][0]);
					} else {
						nAddFrag.appendChild(anRows[i]);
					}
				}
		...
		function _fnInitialise ( oSettings )
		{
			...
			$('div.data-table-length > label > select').selectmenu({'style':'dropdown'});
		...
		this.fnClose = function( nTr )
		{
			...
			for ( var i=0 ; i<oSettings.aoOpenRows.length ; i++ )
			{
				if ( oSettings.aoOpenRows[i].nParent == nTr )
				{
					...
					//return 0;
					oSettings.aoOpenRows[i].nTr.remove();
					oSettings.aoOpenRows.splice(i, 1);
					--i;
					closed = 0;
				}
		...
		this.fnOpen = function( nTr, mHtml, sClass )
		{
			...
			var nNewRow = null;
			var nNewCell = null;
			if(mHtml.substr(0, 3) === '<tr') {
				nNewRow = mHtml;
				
				var prevTr = nTr;
				$(mHtml).each(function(i) {
					var $this = $(this);
					$this.insertAfter(prevTr);
					prevTr = $this;
					oSettings.aoOpenRows.push({
						'nTr': $this,
						'nParent': nTr,
						'nTrParent': $(nTr).parent()
					});
				});
			} else {
		...
		"two_button": {
			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
			{
				...
				var sAppend = (!oSettings.bJUI) ?
					'<a class="'+oSettings.oClasses.sPagePrevDisabled+'" tabindex="'+oSettings.iTabIndex+'" role="button"><span class="paginate-icon"></span><span>'+oLang.sPrevious+'</span></a>'+
					'<a class="'+oSettings.oClasses.sPageNextDisabled+'" tabindex="'+oSettings.iTabIndex+'" role="button"><span>'+oLang.sNext+'</span><span class="paginate-icon"></span></a>'
					:
	...
	$.extend( DataTable.ext.oSort, {
		...
		"formatted-numeric-pre": function (a) {
			return a.match(/\d/) ? a.replace(/[^\d.-]/g, '') : 0;
		},
		
		"formatted-numeric-asc": function (x, y) {
			return x - y;
		},
		
		"formatted-numeric-desc": function (x, y) {
			return y - x;
		}
	} );
